...
function FollowingListPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (ownerRow.hide_following && uid !== ownerRow.id) {
        setBlocked(true);
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-full bg-background">
      <header className="sticky top-0 z-10 flex items-center gap-3 px-4 py-3 bg-background/85 backdrop-blur border-b border-border">
        ...
        <div className="min-w-0">
          ...
          <h1 className="text-base font-bold leading-tight">Seguindo</h1>
        </div>
      </header>
      ...
      {loading ? (
        ...
        <div className="p-8 text-center max-w-md mx-auto">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Você não poderá visualizar quem esta conta está seguindo.
          </p>
        </div>
      ) : rows.length === 0 ? (
